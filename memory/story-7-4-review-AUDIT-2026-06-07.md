...
- **此前为何偏离**：我把焦点放在"7.3 没独立 commit"的事实陈述上，未追问"那这次 commit 应当如何分层"。本质是 *陈述事实但未给出可执行的决策树*。
...
**第一性原理溯源**：
- **原理**：`.gitignore` 只阻止 *未跟踪* 文件；已跟踪文件必须 `git rm --cached` 才能从 index 移除（而工作树保留）。
- **此前为何偏离**：我用一个具体文件名 `test-output.txt` 做了"代表性陈述"，未运行 `git ls-files` 通配检查同名兄弟。这是 *样本偏差*（用 1 个观测代表整族）。
...
- **此前为何偏离**：我看到"Ctrl+Z 有 cancelDrag 模式"就推荐"复用"，未对比两个 handler 的 *完整语义*（Ctrl+Z 保留历史、Rewind 丢弃）。这是 *模式匹配优先于语义对比* 的偏差。
...
- **此前为何偏离**：我用"随节点 GC"做了 *因果省略*，跳过了中间环节"模块替换 → const 失引用"。这是 *单步因果近似多步因果* 的偏差。
