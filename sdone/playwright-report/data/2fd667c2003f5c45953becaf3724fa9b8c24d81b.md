...
- Name: history.test.ts >> History (Undo/Redo) >> [P1] create connection → undo removes it → redo restores it
- Location: e2e\history.test.ts:40:3
...
Call log:
  ...
  - waiting for locator('.rate-editor__form')
    8 × locator resolved to <div class="rate-editor__form">…</div>
...
```ts
  1   | /**
  2   |  * SDONE E2E — History (Undo/Redo) Integration Tests
  3   |  *
  4   |  * Covers: Undo/redo for module creation, move, delete.
  5   |  * Verifies undo/redo integration with countdown panel and rate editor.
  6   |  */
  7   | 
