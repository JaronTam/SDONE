...
- Name: panels.test.ts >> Panels >> [P2] clicking countdown row selects the stock on canvas
- Location: e2e\panels.test.ts:140:3
...
Locator: locator('.countdown-panel__row').first()
Expected: visible
Timeout: 2000ms
...
Call log:
  ...
  - waiting for locator('.countdown-panel__row').first()
...
> 145 |     await expect(row).toBeVisible({ timeout: 2000 });
  ...
  171 | 
  172 |     // Module panel should be visible again
  173 |     const panelVisible = await isModulePanelVisible(page);
  174 |     expect(panelVisible).toBe(true);
  175 |   });
  176 | 
  177 |   test('[P2] P key toggles panel pin state', async ({ page }) => {
  178 |     // Press P to toggle pin
  179 |     await pressKey(page, 'p');
  180 |     await page.waitForTimeout(300);
  181 | 
  182 |     // Run simulation — pinned panel should stay visible
  183 |     await clickRun(page);
  184 |     await page.waitForTimeout(500);
  185 | 
  186 |     const panelVisible = await isModulePanelVisible(page);
  187 |     // Should be visible because pinned
  188 |     expect(panelVisible).toBe(true);
  189 |   });
  190 | });
  191 | 
```