...
function emptyState(): GraphState {
  return { nodes: {}, connections: {}, version: 0, selectedModuleIds: [], selectedConnectionIds: [] };
}
...
function fullPath(s: GraphState, t: ModuleType, x: number, y: number, h: HistoryManager, eb: EventBus): GraphState {
...
describe("NFR-P3 Module Placement Latency (<=16ms frame budget)", () => {
  describe("mutation only", () => {
    it("stock p50<1ms p99<3ms", () => {
      ...
      for (let i=0;i<MEASURE;i++) {
        const s=emptyState(); const t0=performance.now();
  ...
  describe("full path", () => {
    it("stock <5ms x100", () => {
      const h=new HistoryManager(); const eb=new EventBus(); let c=0;
      eb.on("MODULE_PLACED",()=>{c++;});
      const ts:number[]=[];
    ...
    it("source+palette <5ms", () => {
      const h=new HistoryManager(); const eb=new EventBus(); const ts:number[]=[];
    ...
    it("cold start <5ms", () => {
      const h=new HistoryManager(); const eb=new EventBus();
      const s=emptyState();
    ...
    it("populated state <5ms", () => {
      const h=new HistoryManager(); h.push(emptyState());
      const eb=new EventBus();
      let state=emptyState();
      for(let i=0;i<5;i++){state=addModule(state,"source",{x:i*100,y:0});state=addModule(state,"stock",{x:i*100,y:100});}
      ...
      const ts:number[]=[];
      for(let i=0;i<MEASURE;i++){
        const sc={...state,nodes:{...state.nodes},connections:{...state.connections},version:state.version};
        const t0=performance.now();
