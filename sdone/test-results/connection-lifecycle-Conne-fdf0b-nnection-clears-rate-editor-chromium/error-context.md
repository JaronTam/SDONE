...
- Name: connection-lifecycle.test.ts >> Connection Lifecycle >> [P1] selecting a module after a connection clears rate editor
- Location: e2e\connection-lifecycle.test.ts:180:3
...
Call log:
  ...
  - waiting for locator('.rate-editor__form')
    18 × locator resolved to <div class="rate-editor__form">…</div>
...
> 189 |     await expect(page.locator('.rate-editor__form')).toBeVisible({ timeout: 2000 });
  ...
  251 |     const handleX = stockScreen.x - 70; // left edge + FEEDBACK_ARC_OFFSET
  252 |     const handleY = stockScreen.y;
  253 |
  254 |     // Try to click and drag from the feedback handle area toward the source
  255 |     await page.mouse.move(handleX, handleY);
  256 |     await page.mouse.down();
  257 |     // Drag toward source
  258 |     const srcScreen = worldToScreen(SRC_POS.x, SRC_POS.y);
  259 |     await page.mouse.move(srcScreen.x, srcScreen.y, { steps: 8 });
  260 |     await page.mouse.up();
  261 |     await page.waitForTimeout(300);
  262 |
  263 |     // App should remain functional, no crash
